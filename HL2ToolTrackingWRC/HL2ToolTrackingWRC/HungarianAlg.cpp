			...
			for (size_t col = 0; col < nOfColumns; col++)
			{
				...
				if (singleValidationFound)
				{
					for (size_t nestedRow = 0; nestedRow < nOfRows; nestedRow++)
						if ((nOfValidObservations[nestedRow] > 1) && distMatrix[nestedRow + nOfRows * col] != std::numeric_limits<float>::max())
						{
							distMatrix[nestedRow + nOfRows * col] = std::numeric_limits<float>::max();
